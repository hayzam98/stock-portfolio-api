...
def test_get_portfolio_summary_empty(client):
    ...
    response = client.get(
        "/portfolio/summary", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_stock_summary(client):
    ...
    # Get stock summary
    response = client.get(
        "/portfolio/summary/AAPL", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_total_portfolio_value(client):
    ...
    # Get totals
    response = client.get(
        "/portfolio/total", headers={"Authorization": f"Bearer {token}"}
    )
