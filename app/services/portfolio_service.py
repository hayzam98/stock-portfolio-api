...
class PortfolioService:
    ...
    @staticmethod
    def calculate_portfolio_summary(user_id: int, db: Session) -> List[PortfolioSummary]:
        """
        ...
        ).order_by(Transaction.stock_symbol, Transaction.transaction_date).all()
        ...
        for symbol, data in stocks_data.items():
            ...
            if data['total_shares'] > 0:
                ...
                avg_buy_price = (
                    data['total_invested'] / sum(t['quantity'] for t in data['buy_transactions'])
                    if data['buy_transactions'] else 0
                )
    ...
    @staticmethod
    def calculate_stock_summary(user_id: int, stock_symbol: str, db: Session) -> PortfolioSummary:
        """
